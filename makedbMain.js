...
fetch('https://jsadvancedclassproj1.firebaseio.com/user.json', {method: 'DELETE'}).then(() => {
  for (let i = 0; i < 100; i++) {
    ...
    avatarGenerator(md5(firstName + lastName), Math.random() < 0.5 ? 'male' : 'female', 100)
      .toBuffer(function (err, buffer) {
        ...
        fetch('https://jsadvancedclassproj1.firebaseio.com/user.json', pkg).then(console.log).catch(console.log)
      })
  }
})
